...
function StatBigCard({ icon, tone, value, label, suffix }) {
  return (
    <Card tone="lowest" style={{ flex: 1, padding: 18 }}>
      ...
      <div style={{ font: '800 32px var(--font-display)', color: 'var(--md-on-surface)', marginTop: 12, lineHeight: 1 }}>
        {value}<span style={{ font: '700 16px var(--font-body)', color: 'var(--md-on-surface-variant)' }}>{suffix}</span>
      </div>
      <div style={{ font: '500 13px var(--font-body)', color: 'var(--md-on-surface-variant)', marginTop: 5 }}>{label}</div>
    </Card>
  );
}
  ...
  return (
    <div className="screen-pad">
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 22 }}>
        ...
        <StatBigCard icon="trophy" tone="tertiary" value={rate} suffix="%" label="אחוז השלמה" />
      </div>
      ...
      <Card tone="lowest" className="reveal" style={{ padding: 20, marginBottom: 22 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
          ...
          <div style={{ font: '600 13px var(--font-body)', color: 'var(--md-primary)' }}>{`${totalDone} מתוך ${totalAll}`}</div>
        </div>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 8, height: 170, paddingTop: 16 }}>
          {WEEK_STATS.map((d, i) => {
            ...
            return (
              <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, height: '100%' }}>
                <div style={{ flex: 1, width: '100%', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
                  <div style={{
                    width: '72%', height: h + '%', borderRadius: 'var(--r-sm)', position: 'relative',
                    background: 'var(--md-surface-container-highest)', overflow: 'hidden',
                  }}>
                    <div className="bar-grow" style={{
                      position: 'absolute', insetInline: 0, bottom: 0, height: (dh / h * 100) + '%',
                      background: 'var(--md-primary)', borderRadius: 'var(--r-sm)',
                      animationDelay: (i * 70) + 'ms',
                    }} />
                  </div>
                </div>
                <span style={{ font: '700 13px var(--font-body)', color: 'var(--md-on-surface-variant)' }}>{d.day}</span>
              </div>
            );
          })}
        </div>
          ...
          <Card key={row.label} tone="lowest" style={{ padding: 14, display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ height: 8, borderRadius: 'var(--r-pill)', background: 'var(--md-surface-container-highest)', overflow: 'hidden' }}>
                <div className="bar-grow-x" style={{
                  height: '100%', width: row.val + '%', borderRadius: 'var(--r-pill)',
                  background: row.tone === 'tertiary' ? 'var(--md-tertiary)' : row.tone === 'error' ? 'var(--md-error)' : 'var(--md-primary)',
                }} />
              </div>
            </div>
          </Card>
